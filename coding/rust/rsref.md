...
-   `trait` 特性：描述类型可以实现的抽象接口（组合），以定义实现某些目的所需一组的行为
    -   `trait` 特性可以包含以下 3 种 *关联项*
        ...
        -   常量 `const`：关联常量
    -   `trait` 需要具体类型实现，无默认值函数、常量需要具体类型实现
...
> - `src/core/marker`：<https://doc.rust-lang.org/src/core/marker.rs.html>
    ...
    -   对只包含栈上内存（大小、类型已知）的值，*Rust* 执行复制 `clone`，否则 **执行移动 `move`**
        ...
        -   `Copy trait`：实现 `Copy` 特性的类型值将被简单复制而不是移动
            -   `Copy` 特性（可）被用于存储在栈上的数据
                -   任何简单标量类型（可以）实现 `Copy`
                -   只包含实现 `Copy` 类型的元组
            -   类型实现、或任意成员实现 `Drop` 特性则不可使用 `Copy` 标记
...
| 智能指针                    | 说明                                                   | 线程安全版            |
|-----------------------------|--------------------------------------------------------|-----------------------|
| `alloc::boxed::Box<T>`      | 堆内存分配、封装，对齐非同类数据                       |                       |
| `alloc::rc::Rc<T>`          | 引用计数器，共享数据                                   | `std::sync::Arc<T>`   |
| `alloc::rc::Weak<T>`        | 弱引用计数，由 `Rc<T>::downgrade()` 避免引用循环       |                       |
| `core::cell::Cell<T>`       | 内部 `Copy` 类型值可变性，作为结构体成员独立可变       |                       |
| `core::cell::RefCell<T>`    | 内部非 `Copy` 类型值可变性，配合 `Rc<T>` 共享可变数据  | `std::sync::Mutex<T>` |
...
> - `futures_task::arc_wake::ArcWake`：<https://docs.rs/futures/latest/futures/task/trait.ArcWake.html>
...
> - `futures::waker`：<https://docs.rs/futures-task/0.3.31/src/futures_task/waker.rs.html>
> - `core::task::wake`：<https://doc.rust-lang.org/src/core/task/wake.rs.html>
