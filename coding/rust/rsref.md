...
let arr: [i32; 5] = [1, 2, 3, 4, 5];      // 显式指定数组类型
...
-   *Coercions* （类型）强制转换：在某些上下文中，类型可以被隐式的转换为其他类型
    ...
    -   但，强制转换 **不会被应用以满足** *Trait Bound* （除方法中 `self` 自动类型转换）
        -   即，若 `U: impl Trait` 且 `T` 可强制转换为 `U`，但不代表 `T` 可适用需要 `impl Trait` 类型场合
        -   常见的，函数形参要求 `t: impl Trait`、且 `impl Trait for &T`，但 `&mut T` 不可满足形参 `t` 要求
...
-   结构体：将相关的值打包、命名、形成有意义组合的自定义数据类型
    ...
    -   结构体类型、枚举类型通过 `impl` 块（可分多块）定义关联函数
        -   *Method* 方法：首个形参为 `self` 类实例自身的关联函数
            ...
            -   但，编译器对方法中 `self` 有特殊的类型转换机制，对完全限定语法不适用
        -   狭义关联函数：非方法的关联函数
...
-   *Mark Trait* 标记特性：仅用于 *Trait Bound*，即用于编译器检查某类型是否满足要求
    ...
    -   *Negative Trait* 负面特性 `!`：标记类型不具有某特性，如 `!Send`、`!Sync`
...
-   内存布局
    -   *Alignment* 对齐：若某类型对齐为 `n`，则存储其值的有效地址必须为 `n` 的整数倍
        -   类型大小必须是对齐的整数倍
            ...
            -   不对齐将因缓存行分割而导致性能下降，在某些平台上可能直接崩溃
            -   对齐只能增大（以优化缓存效率），不能减少
        -   可通过 `std::mem::align_of::<T>()` 确定类型 `T` 的对齐
            -   基本类型：通常按类型大小对齐（但与平台相关）
                -   `u32` 按 4B 对齐
                -   `u64` 按 8B 对齐
                -   `i8` 按 1B 对齐
            -   复合结构类型：整体、结构体内各字段均需对齐
                -   类型整体需 **按对齐最大字段对齐**
                -   且，结构成员字段只需保证按自身对齐
                -   为此，*Rust* 会在必要时会重排字段、*padding* 补齐以将字段对齐
    -   `std::alloc::Layout` 内存布局：描述内存块需求，用于向内存全局分配器申请内存时描述内存需求
        -   内部布局由 `size` 字节数、`align` 对齐两个字段描述
    -   可通过 `#[repr()]` 属性宏手动干预内存布局（对齐）
        -   `#[repr(align(N))]` 指定对齐下限：默认对齐大于 `N` 时不生效
        -   `#[repr(packed(N))]` 压缩对齐：默认对齐大于 `N` 也生效
            -   `#[repr(packed)]` 等价于 `#[repr(packed(1))]`
        -   `#[repr(C)]` C 语言兼容布局：强制 C 风格的按字段声明顺序布局
        -   `#[repr(u8)]`、`#[repr(i32)]` 枚举底层类型：枚举类型使用 `u8`、`i32` 存储判别值
        -   `#[repr(transparent)]` 透明包装：单字段结构体，与内部类型同布局
        -   `#[repr(C, align(16))]` 组合使用：C 语言兼容、16B 对齐内存布局
    -   事实上，除数组外，*Rust* 不保证字段顺序、完整性
...
-   *Destructor* 析构（器）
    ...
    -   *Rust* 严格按 *RAII* 规范，在变量被销毁时自动析构，类型 `T` 值的析构过程包括
        -   若 `T: Drop`，调用 `<T as core::ops::Drop>::drop` 执行额外清理逻辑（清理堆内存）
            -   故，不允许手动显式调用 `Drop::drop` 方法，否则可能导致 *Double Free* 而 `panic`
        -   *Drop Glue*：递归的对类型 `T` 包含字段析构
            -   结构体、枚举变体：字段按声明顺序丢弃（与 *C/C++* 相反）
            -   元组、数组、拥有所有权的切片：元素按顺序丢弃
            -   闭包捕获的变量丢弃顺序不固定
            -   *Trait 对象* 执行底层类型析构
            -   其他类型不执行额外操作
...
-   `ops::Drop`：自定义类型变量析构（生命周期结束）时，需要额外执行的清理逻辑（若无则无需实现）
    -   典型即用于释放 **自身（不考虑内部成员）** 资源
        -   故，`Drop::drop` 常用于释放拥有的堆上内存、外部资源
            -   若方法涉及 `alloc::alloc` 堆上内存至裸指针，`Drop::drop` 中需要 `alloc::dealloc`
            -   若方法涉及向堆内存写入、移入 `impl Drop` 类型值，`Drop::drop` 中逐项 `drop`
            -   若拥有的外部资源，`Drop::drop` 中需调用外部资源提供的 *API* 释放内存
        -   其他常见需 `impl Drop` 场景包括
            -   修改引用计数
            -   作用域守卫：变量退出作用域时必须执行的逻辑
        -   故，一般仅在类型包含 **需分配内存裸指针**、与外部资源交互时需 `impl Drop`
    -   其他事项
        -   `Drop` 与 `Copy` 标记特性不兼容
            -   `impl Drop` 将自动移除 `Copy` 标记特性
        -   *Rust* **禁止 `impl Drop` 类型执行部分移动**
            -   析构需要完整状态，部分移动可能导致类型值整体析构失败
...
> - *Implementing Vec - Deallocating*：<https://doc.rust-lang.net.cn/nomicon/vec/vec-dealloc.html>
...
| `ptr::Unique<T: ?Szied>`  | 逻辑上拥有数据数据 `T` 的 `NonNull<T>` |
...
-   裸指针封装类型
    ...
    -   `ptr::Unique<T>` 则进一步封装 `NonNull<T>`
        -   表明指针逻辑上拥有数据 `T`（但实际上不负责数据 `T` 资源管理）
        -   与数据 `T` 拥有相同 `Send`、`Sync` 特质
