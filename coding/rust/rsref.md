...
-   *Reference* 引用（类型） `&`：可用于访问其包含（指向）地址上存储的数据的指针
    -   引用仅仅 **借用其指向的（所引用的变量绑定的）值**
        ...
        -   引用没有其他特殊功能，也没有额外开销
    -   切片引用（类型）`&[T]`、`&mut [T]`：引用集合中连续元素序列（切片）的引用
        -   **切片 `[T]` 本身是 *DST* 动态大小类型（动态长度）**
            -   相对应的，数组 `[T; N]` 是固定长度类型
        -   故，常用切片引用而不是切片本身，且 “切片” 也更多指 “切片引用”
            -   `&str` 字符串字面量：指向二进制文件中特定位置的切片
            -   `&[i32]` 数组切片
...
-   注意，编译器的引用检查 **局限在单个函数内部的 `&`、`&mut` 引用类型（操作符）**（显式借用、方法中 `self`）
    ...
    -   此时，需要智能指针 **自行实现运行时单一可变引用检查**
        -   `RefCell<T>.borrow_mut(&self)` 多次调用、创建、返回多个 `RefMut<T>` 可正常通过编译
            -   其参数为自身的不可变引用、且 `RefMut<T>` 中无引用（**无生命周期参数**）
