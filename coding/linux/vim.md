...
| `N` | `[(` | 向后至第 `N` 个未闭合的 `(`                               |
| `N` | `[{` | 向后至第 `N` 个未闭合的 `{`                               |
| `N` | `[m` | 向后至第 `N` 个方法（`method`） 的开始（用于 *Java* 等）  |
| `N` | `[M` | 向后至第 `N` 个方法的结束（`method`）（用于 *Java* 等）   |
| `N` | `])` | 向前至第 `N` 个未闭合的 `)`                               |
| `N` | `]}` | 向前至第 `N` 个未闭合的 `}`                               |
| `N` | `]m` | 向前至第 `N` 个方法（`method`） 的开始（用于 *Java* 等）  |
| `N` | `]M` | 向前至第 `N` 个方法的结束（`Method`） （用于 *Java* 等）  |
...
-   *Vim* 中文本对象说明
    -   句子、段落、小节
        -   句子：以 `.!?` 结尾并紧随换行、空格、制表符
        -   段落：从空行、或段落宏命令开始
            -   段落宏由 `paragraphs` 选项中成对出现字符定义
        -   小节：从首列出现的换页符、或小节宏命令开始
            -   小节宏由 `sections` 选项中成对出现字符定义
...
|     | `:{range}ma[rk] {a-zA-Z'}` | 在 `{range}` 末行 0 列设置位置标记，缺省当前行             |
|     | `:{range}k {a-zA-Z'}`      | 同 `:mark`                                                 |
|     | `:marks {mark}`            | 列出位置标记                                               |
|     | `:loc[kmarks] {cmd}`       | 不调整位置标记的情况下执行命令，适用于不影响行数的改动     |
|     | `:kee[pmarks] {cmd}`       | 保持（过滤）命令跳转标记不变（若过滤后行数不减）           |
|     | `'{a-zA-Z0-9[]'"<>.}`      | `'` 同 <code>\`</code>，但同时移动至该行的首个非空白字符上 |
|     | `:ju[mps]`                 | 列出当前窗口跳转表                                         |
|     | `:cle[arjumps]`            | 清除当前窗口跳转表                                         |
|     | `:keepj[jumps]`            | 清除当前窗口跳转表                                         |
| `N` | `CTRL-o`                   | 跳转至当前窗口跳转表中第 `N` 较老跳转处                    |
| `N` | `CTRL-i`、`<TAB>`          | 跳转至当前窗口跳转表中第 `N` 较新跳转处                    |
|     | `:changes`                 | 列出当前窗口修改表                                         |
| `N` | `g;`                       | 跳转至当前窗口修改表第 `N` 较新修改处                      |
| `N` | `g,`                       | 跳转至当前窗口修改表第 `N` 较旧修改处                      |

-   位置标记说明
    -   位置标记除 `a-zA-Z'` 外都由 *Vim* 自动设置
    -   跳转表：记录光标移动行外的命令导致的光标移动的位置表，不包括行内查找、左右移动
        -   跳转表：记录跳转（光标移至数行外）位置
            -   每个窗口有独立跳转表，最多存储 100 项
            -   跳转表包含窗口中不同缓冲区的跳转位置
            -   跳转表会复制到被分隔的窗口中
            -   同一行的新跳转会删除旧的跳转
        -   跳转时，跳转表中 `>` 指示在当前跳转表所在位置
    -   修改表：记录文本改变的位置的表
        -   同一行内、列差小于 `textwidth` 的两个可撤销的改变中，仅有较新者被记录

| 位置标记             | 说明                                                       |
|----------------------|------------------------------------------------------------|
| `{a-z}`              | 文件内位置，单个文件内有效，缓冲区卸载时丢失               |
| `{A-Z}`              | 会话内可跨文件位置，可存储在 *viminfo* 中                  |
| `{0-9}`              | 依次距今更远的、离开 *Vim* 时的位置，可存储在 *viminfo* 中 |
...
| `N` | 命令                  | 说明                                             |
|-----|-----------------------|--------------------------------------------------|
| `N` | `v`                   | （`N` 倍选择上次）以字符方式开始高亮             |
| `N` | `V`                   | （`N` 倍选择上次） 以行方式开始高亮              |
| `N` | `CTRL-V`              | （`N` 倍选择上次） 以列块方式开始高亮            |
| `v` | `v_o`                 | 交换高亮区域的开始处和光标位置                   |
|     | `gv`                  | 在前次可视区域上开始高亮                         |
|     | `gn`                  | 使用前次搜索模式正向匹配，并启动可视模式选中匹配 |
| `v` | `v_gn`                | 扩展至下个匹配，并选中中途未匹配部分             |
|     | `gN`                  | 使用前次搜索模式逆向匹配，并启动可视模式选中匹配 |
| `v` | `v_gN`                | 扩展至下个匹配，并选中中途未匹配部分             |
| `v` | `v_v`                 | 以字符方式开始高亮或停止高亮                     |
| `v` | `v_V`                 | 以行方式开始高亮或停止高亮                       |
| `v` | `v_CTRL-V`            | 以列块方式开始高亮或停止高亮                     |
| `v` | `v_g CTRL-g`          | 显示可视区域单词、字符、行、字节计数             |
| `v` | `v_<ESC>`、`v_CTRL-C` | 终止可视模式                                     |

-   可视对象：移动光标或使用操作符来作用于高亮的文本
    -   可视模式的流程
        -   `v`、`V`、`CTRL-v` 进入 *Visual* 模式，标记文本开始
        -   移动光标至目标位置，开始、目标位置间文本高亮显示
        -   键入命令，应用于高亮文本
    -   可视模式指定一定范围，适用删除、修改、替换、连接、命令执行等
        -   支持类似 *操作符-动作* 的模式
            -   可与操作符命令连用
            -   支持文本对象选择、单词匹配选择范围
        -   支持前缀计数、寄存器存储数据
    ...
    -   可视模式下不能使用 *Ex* 命令、可能改变（放弃）文件的命令，修改可视区域范围

| *Vim* 选项    | 说明                               |
|---------------|------------------------------------|
| `selection`   | 光标选择范围（是否包含光标下字符） |
| `virtualedit` | 光标可移至任意位置                 |

####    `Visaul Block` 模式插入、修改

| `N`  | 命令   | 说明                                               |
|------|--------|----------------------------------------------------|
| `vb` | `vb_I` | 在所有选中的行之前插入相同的文本                   |
| `vb` | `vb_A` | 在所有选中的行之后添加相同的文本                   |
| `vb` | `vb_c` | 选择区域所有文本用相同字符串替代                   |
| `vb` | `vb_C` | 同 `v_c`，但选择区域扩展至每行                     |
| `vb` | `vb_>` | 列块按 `shiftwidth` 指定单位移动                   |
| `vb` | `vb_r` | 使用相同字符替代列块                               |
| `vb` | `vb_$` | 列块模式下，选择全部行整行（此时各行选中长度不同） |
...
-   *操作符-动作*：操作符后跟动作命令 `{motion}`，操作符被作用于所跨越文本上
...
| 命令                                | 说明                                                |
|-------------------------------------|-----------------------------------------------------|
| `:reg[isters] [args]`、`:di[splay]` | 列出寄存器内容                                      |
| `"{char}`                           | 在接下来的删除、抽出或放置命令中使用寄存器 `{char}` |
| `:"{reg}d{motion}`                  | 删除并存储原内容至寄存器 `{reg}` 中                 |
| `:"{reg}c{motion}`                  | 修改并存储原内容至寄存器 `{reg}` 中                 |

-   寄存器：暂时存储编辑过程中的删除、修改、复制、命令执行的内容
    -   删除、修改、复制、命令执行的内容有对应的专用寄存器
    -   此外在执行动作前，可显示指定具名 `"{a-zA-Z}` 寄存器存储对应内容
        -   删除：`d`、`D`、`:delete`、`:deletel`、`:deletep`
        -   修改：`c`、`C`、`S`、`R`

| 寄存器        | 说明                                                            |
|---------------|-----------------------------------------------------------------|
| `"`           | 无名寄存器，删除、抽取命令文本存储于此（即使指定 `_` 外寄存器） |
...
| `a-z`         | 具名寄存器，替换寄存器中内容                                    |
| `A-Z`         | 具名寄存器、添加至寄存器中内容后                                |
| `.`           | （只读）存储最近插入文本                                        |
| `:`           | （只读）存储最近命令行命令                                      |
| `%`           | （只读）存储当前文件名                                          |
| `#`           | 轮换缓冲区寄存器                                                |
| `=`           | 表达式寄存器，用于在使用寄存器的命令中使用表达式                |
| `*`           | 选择寄存器，保存、取得 *GUI* 界面上选择的文本                   |
| `+`           | 剪切寄存器，保存、取得 *GUI* 界面上选择的文本                   |
| `~`           | 拖放寄存器，保存、取得 *GUI* 界面上选择的文本                   |
...
| `v` | `v_J`                            | 连接高亮的行                                |
| `N` | `gJ`                             | 同 `J`，但不插入空格                        |
| `v` | `v_gJ`                           | 同 `{visual}J`，但不插入空格                |
|     | `:[range]d [x]`                  | 删除范围 `[range]` 覆盖的行，存入寄存器     |
|     | `:[range]d[elete] {reg} {count}` | 删除 `[range]` 覆盖的行，存入寄存器 `{reg}` |
|     | `:[range]j[oin][!] [N] [flags]`  | 连接 `[range]` 开始的 `[N]` 行              |
...
| `N` | `C`          | 修改至行尾（及 `N-1` 后续行）                     |
| `N` | `s`          | 修改 `N` 个字符                                   |
| `v` | `c`          | 在可视列块模式下 用键入的文本修改选中文本的每一行 |
| `v` | `C`          | 在可视列块模式下 用键入的文本修改选中各行直至行末 |
| `N` | `~`          | 反转 `N` 个字符的大小写并前进光标                 |
| `v` | `~`          | 反转高亮文本的大小写                              |
| `v` | `u`          | 修改高亮的文本为小写                              |
| `v` | `U`          | 修改高亮的文本为大写                              |
|     | `g~{motion}` | 反转动作 `{motion}` 跨越的文本的大小写            |
|     | `gu{motion}` | 修改动作 `{motion}` 跨越的文本为小写              |
|     | `gU{motion}` | 修改动作 `{motion}` 跨越的文本为大写              |
...
|      | `:[range]< [N] [flags]`    | `{range}` 开始后续 `N` 行左移                        |
| `N`  | `>{motion}`                | 右移动作 `{motion}` 跨越的多行一个 `shiftwidth`      |
| `N`  | `>>`                       | 右移 `N` 行一个 `shiftwidth`                         |
|      | `:[range]> [N] [flags]`    | `{range}` 开始后续 `N` 行右移                        |
| `Nv` | `v_gq{motion}`             | 排版动作 `{motion}` 跨越的多行到 `textwidth` 宽      |
...
|     | `:so[urce] {file}`                  | 从文件 `{file}` 读入 *Ex* 命令并执行                                         |
|     | `:so[urce]! {file}`                 | 从文件 `{file}` 读入 *VIM* 命令并执行                                        |
...
|-----|-------------------------|-------------------------------------------------------|
| `N` | `u`                     | 撤销最近的 `N` 此改动                                 |
|     | `:u[ndo] {num}`         | 撤销至更改树中第 `{num}` 节点（缺省父节点、前一节点） |
|     | `:unodl[ist]`           | 列出更改树的叶子节点                                  |
| `N` | `CTRL-R`                | 重做最近的 `N` 个被撤销的改动                         |
|     | `:red[o]`               | 重做最近一次被撤销更改                                |
|     | `U`                     | 恢复最近被改动的行                                    |
| `N` | `g-`                    | 跳转到撤销树中前 `N` 个较早动作状态                   |
| `N` | `g+`                    | 跳转到撤销树中后 `N` 个较晚动作状态                   |
|     | `:earlier {time}{unit}` | 跳转到撤销树中前 `time-unit` 时间点较早动作状态       |
|     | `:later {time}{unit}`   | 跳转到撤销树中后 `time-unit` 时间点较晚动作状态       |
| `i` | `i_CTRL-g u`            | 插入撤销断点                                          |
|     | `:undoj[oin]`           | 合并撤销断点                                          |
|     | `:wundo[!] {file}`      | 将撤销历史写入撤销文件 `{file}`                       |
|     | `:rundo[!] {file}`      | 从撤销文件 `{file}` 读入撤销历史                      |

-   说明
    -   撤销断点可用于在连续输入较长文本时，分隔撤销动作影响
    -   撤销树：*Vim* 将每次更改、撤销以树节点的方式组织
        -   `:undolist` 仅展示叶子节点
            -	`number`：叶节点编号
            -	`changes`：树根节点到此叶节点的改变数目
            -	`when`：改变发生时间
            -	`saved`：第几次写入文件
        -   `g-`、`g+` 在撤销树中所有节点中、按时间顺序跳转
        -   `u`、`CTRL-R` 仅在撤销树中当前分支（叶子节点到根节点）上跳转

| *Vim* 选项   | 说明                   |
|--------------|------------------------|
| `undolevels` | 可记录的最大改变次数   |
| `undofile`   | 是否保存撤销记录至文本 |
| `undodir`    | 撤销记录文件目录       |

> - *Ch10 - 撤销*：<https://docs.kilvn.com/vim_zh_cn/ch10_undo_zh.html>
...
| 命令                       | 说明                                      |
|----------------------------|-------------------------------------------|
| `:se[t] [!]`               | 显示所有改动过的选项（`!` 单行显示）      |
| `:se[t] all`               | 显示所有非 `termcap` 选项（`!` 单行显示） |
...
| `:se[t] {option}&vi`       | 重置 `{option}` 为 *Vi* 模式缺省值        |
| `:se[t] {option}&vim`      | 重置 `{option}` 为 *Vim* 模式缺省值       |
| `:se[t] all&`              | 重置所有非 `termcap` 为其缺省值           |
| `:setl[ocal] ...`              | 同 `set`，但对局部选项设定其局部值        |
| `:setl[ocal] all`          | 显示所有局部选项的局部值|
| `:setg[lobal]`             | 同 `set`，但对局部选项仅修改其全局值        |
| `:fix[del]`                | 根据 `t_kb` 的值来设置 `t_kD`             |
| `:opt[ions]`               | 打开一个新窗口，用来参看并设置选项        |
| `:se[t] {option}<`              | 撤销 `{option}` 局部值以使用全局值|
| `:setl[ocal] {option}<`              | 复制全局值作为局部值|
...
> - *VIM option viminfo*：<https://yianwillis.github.io/vimcdoc/doc/options.html>
...
-   状态栏相关选项
    -   `statusline`：设置状态栏内容
        ...
        set statusline+=%L
        ```
        -   选项值可以包含 `printf` 风格的 `%` 项目：`%{-}{MIN_WIDTH}.{MAX_WIDTH}{ITEM}`
        -   项目格式代码
...
def cover_term(
    lines: str,
    pattern: str = None,
    star_terms: list[str] = None,
) -> str:
    if pattern is None:
        # ch = r"a-zA-Z_\d"
        ch = r"\w"
        si = r"\-<>:\$/\[\]\(\)\{\}%"
        pattern = rf"([{ch}{si}]*[{ch}]+[{ch}{si}]*)"
    if star_terms is None:
        star_terms = {
            "python": "Python",
            "vim": "VIM",
            "c/c++": "C/C++",
        ...
        star_terms = set(star_terms)
    master_ptn = re.compile(pattern, re.A)

    def _replace(mt):
        se, = mt.groups()
        if se.lower() in star_terms:
            return f"*{star_terms[se.lower()]}*"
        else:
            return f"`{se}`"

    return master_ptn.sub(_replace, lines)
...
def md_table(
    ...
    for ll in lines.split("\n"):
        ...
        if mt is not None:
            ...
            d = cover_term(d)
            al = max(len(a.encode("gbk")), al)
            bl = max(len(b.encode("gbk")), bl)
            cl = max(len(c.encode("gbk")), cl)
            dl = max(len(d.encode("gbk")), dl)
            cts.append((a, b, c, d))
    sts = []
    for a, b, c, d in cts:
        b = f"`{b}`" if b else ""
        c = f"`{c}`"
        st = f"| {b.ljust(bl+2)} | {c.ljust(cl+2)} | {d.ljust(dl)} |"
        sts.append(st)
