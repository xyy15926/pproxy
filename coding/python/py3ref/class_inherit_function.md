...
-    `obj.__instancecheck__(self, instance)`：判断 `instance` 是否为 `self` 实例
    -    元类普通方法（`abc.ABCMeta` 中即实现此方法）
        ...
        -    `instance`：待判断实例
        -    返回：布尔值
    -    钩子函数：`isinstance(instance, class)`
...
-    属性获取 `.`：按以下优先级顺序尝试根据属性名获取属性
    ...
    -    对象类型按 *MRO* 查找 `__dict__` 中键对应值
        -    默认的，至此也未查找到属性时，将 `raise AttributeError`
...
-    `obj.__set_name__(self, owner, name)`：描述器初始化时自动（`type` 构造器中）调用
    -    普通实例方法
        -    `owner`：描述器所有者类
