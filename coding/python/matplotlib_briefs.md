...
-    *Matplotlib* 中 *API* 可以分为 3 层
    ...
    -    `matplotlib.backend_bases.Render`：渲染器，在 `FigureCanvas` 上绘图
...
| `set_visible(b)`                                | 可见性                       | `bool D=0`                         | 属性 `_visible`                    |
...
| `set_axis_off()`                                    | 隐藏轴          |             | 无 `get_` |
| `set_axis_on()`                                     | 展示轴          |             | 无 `get`  |
...
-    *Constrained Layout* 限制布局：在满足布局逻辑要求情况下，自动调整组件布局至最优
    ...
    -    须在 `Axes` 添加至图窗之前激活配置
        -    `rcParams` 全局配置：父键 `figure.contrained_layout`
            -    `use`：启用限制布局
            -    `wspace`、`hspace`：`Axes` 间水平、竖直空白大小总和，由各 `Axes` 间平分
