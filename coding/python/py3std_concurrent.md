...
-   `threading.Lock` 基于各平台的内核锁实现（*CPython* 发行版中）
    -   实现层级
        ...
        -   `threading.py` 调用 `_threadmodule.c` 线程、锁并封装为 Python 对象
...
-   `multiprocessing.Process` 沿用了 `threading.Thread` 的 *API*
    -   通过 `threading.Thread` 创建线程有两种主要方式
...
-   事实上，`mp` 中 *API* 本身就是 `mp.context.DefaultContext(BaseContext)` 中方法
    -   `mp` *API* 通过 `mp.context._default_context` 模块级实例导出
    -   `DefaultContext` 有独特的方法（重载、新增）
    ...
    -   而 `mp.context.Process` 被定义为单独类型
        -   额外方法即 `_default_context.get_context().Process` 方法
...
-   同一上下文创建的多进程对象应配合使用，不应跨上下文混用
    ...
    -   且 `mp.synchronize.SemLock.__getstate__` 方法做两个检查，确保 `SemLock` 仅在 `spawn` 进程间序列化、传递
...
-   关于进程池、线程池的说明
    ...
    -   `threading`、`multiprocessing`（自身） 模块被设计为仅提供 `Process`、`Thread`、`Lock` 等底层原语
        ...
        -   早期并无线程池设计：线程创建成本较低，池化收益不如进程，且受多线程受 *GIL* 限制应用范围有限
            -   后续设计有 `multiprocessing.pool.ThreadPool` 类（但不稳定）兼容 `Pool` *API*
