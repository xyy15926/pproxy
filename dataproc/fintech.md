...
-   从维度模型角度看关联网络图谱构建
    ...
    -   事实表描述实体间关系，表中记录对应图中的边
        ...
        -   每条记录对应关系，即图中边
        -   各列为关系属性
    -   关联网络构建方式应根据需求设计，关键在于需要考察的实体、关系
        -   理论上，实体表中每列都可以独立类型顶点加入图谱
...
-   信贷账户详细信息是人行征信内容主体，涉及段如下
    ...
    -   `PCA:PD02` 授信协议信息：主体从数据提供机构获得的授信额度信息的载体
        -   通过 `PD02AI01`、`PD01AI04` *授信协议编号* 与账户关联
...
-   信贷账户各报文段整体账户可见性
    -   `PDA:PD01[]:PD01A` 账户基本信息：所有账户
    -   `PDA:PD01[]:PD01B` 账户最新表现：所有账户
    -   `PDA:PD01[]:PD01C` 账户最近月度：除 *C1* 账户
    -   `PDA:PD01[]:PD01D` 账户近 24 个月还款：除 *C1* 账户
    -   `PDA:PD01[]:PD01E` 账户近 60 个月还款：除 *C1* 账户
    -   `PDA:PD01[]:PD01F` 账户特殊交易：所有账户
    -   `PDA:PD01[]:PD01G` 账户特殊事件：*R2* 账户
    -   `PDA:PD01[]:PD01H` 账户大额专项分期：*R2* 账户
...
|------------|----------------------|------------|--------------------------------------------------------------------|
| `PD01AD01` | 账户类型             | all        |                                                                    |
| `PD01AD02` | 业务管理机构类型     | all        |                                                                    |
| `PD01AD03` | 业务种类             | 除R3       | 有时 “贷记卡-81”会被错报？授信额度为 0                             |
| `PD01AD04` | 币种                 | 除C1       |                                                                    |
| `PD01AD05` | 还款方式             | D1,R1,R4   | “90-不区分还款方式”表汇总报送账户，不区分还款方式                  |
| `PD01AD06` | 还款频率             | D1,R1,R4   | 还款方式为“90-不区分还款方式”时，返回“03-月”                       |
| `PD01AD07` | 担保方式             | 除C1       |                                                                    |
| `PD01AD08` | 贷款发放形式         | D1         |                                                                    |
| `PD01AD09` | 共同借款标志         | D1,R1,R4   |                                                                    |
| `PD01AD10` | 债权转移时的还款状态 | C1         |                                                                    |
| `PD01AI01` | 账户编号             | all        | 报告内唯一                                                         |
| `PD01AI02` | 业务管理机构代码     | all        | 非本机构为空                                                       |
| `PD01AI03` | 账户标识             | 除C1       | 征信系统内全局唯一，非本机构账户为空                               |
| `PD01AI04` | 授信协议编号         | 除C1       | 报告内唯一                                                         |
| `PD01AJ01` | 借款金额             | D1,R4,C1   |                                                                    |
| `PD01AJ02` | 账户授信额度         | R1,R2,R3   |                                                                    |
| `PD01AJ03` | 共享授信额度         | R2,R3      | 在授信机构内的共享授信额度                                         |
| `PD01AR01` | 账户开立日期         | all        |                                                                    |
| `PD01AR02` | 账户到期日期         | D1,R1,R4   | D1R4指最后还款期，可按实际情况调整；R1指额度有效期；“结清”账户为空 |
| `PD01AS01` | 还款期数             | D1,R4      | 非分期还款账户返回空                                               |

| 字段名     | 含义                      | 账户可见性 | 备注                         |
|------------|---------------------------|------------|------------------------------|
| `PD01BD01` | 最新表现_账户状态         | all        | C1、非持续更新账户           |
| `PD01BD03` | 最新表现_五级分类         | D1,R1,R4   | 非（关闭、呆账）账户         |
| `PD01BD04` | 最新表现_还款状态         | 除C1       | 持续更新账户                 |
| `PD01BJ01` | 最新表现_账户余额         | all        | 非关闭账户                   |
| `PD01BJ02` | 最新表现_最近一次还款金额 | 除C1       | 持续更新、且有非月度报送账户 |
| `PD01BR01` | 最新表现_账户关闭日期     | all        | 关闭账户                     |
| `PD01BR02` | 最新表现_最近一次还款日期 | all        | 非（未激活、关闭）账户       |
| `PD01BR03` | 最新表现_信息报告日期     | all        | 持续更新账户同月度段         |
| `PD01BR04` | 最新表现_转出月份         | D1         | 仅转出账户非空               |

| 字段名     | 含义                            | 账户可见性  | 备注                                         |
|------------|---------------------------------|-------------|----------------------------------------------|
| `PD01CD01` | 最近月度_账户状态               | 除C1        | 持续更新、非C1账户才有此段                   |
| `PD01CD02` | 最近月度_五级分类               | D1,R1,R4    |                                              |
| `PD01CJ01` | 最近月度_账户余额               | 除C1        | 对R2：= PD01CJ02 + PD01CJ03 + 利息、费用等   |
| `PD01CJ02` | 最近月度_已用额度               | R2          | 占用循环额度部分（包含已占用但未出单部分）   |
| `PD01CJ03` | 最近月度_未出单大额专项分期余额 | R2          | 未出单：未在账单中体现部分                   |
| `PD01CJ04` | 最近月度_本月应还款             | D1,R1,R2,R4 | D1R41到期前指月度应还总额，到期后指欠款总额，R2账户指出单日计算出的最低还款额 |
| `PD01CJ05` | 最近月度_本月实还款             | 除C1        | 两个月结日、账单日实际还款额总和，即可以大于应还，尤其是R2                    |
| `PD01CJ06` | 最近月度_当前逾期总额           | D1,R1,R2,R4 |                                              |
| `PD01CJ07` | 最近月度_逾期31-60天未还本金    | D1,R1,R4    |                                              |
| `PD01CJ08` | 最近月度_逾期61-90天未还本金    | D1,R1,R4    |                                              |
| `PD01CJ09` | 最近月度_逾期91-180天未还本金   | D1,R1,R4    |                                              |
| `PD01CJ10` | 最近月度_逾期180天以上未还本金  | D1,R1,R4    |                                              |
| `PD01CJ11` | 最近月度_透支180天以上未付余额  | R3          |                                              |
| `PD01CJ12` | 最近月度_最近6个月平均使用额度  | R2          |                                              |
| `PD01CJ13` | 最近月度_最近6个月平均透支余额  | R3          |                                              |
| `PD01CJ14` | 最近月度_最大使用额度           | R2          |                                              |
| `PD01CJ15` | 最近月度_最大透支余额           | R3          |                                              |
| `PD01CR01` | 最近月度_月份                   | 除C1        | 此字段数据可能缺失，建议使用 PD01CR04        |
| `PD01CR02` | 最近月度_本月应还款日           | 除C1        |                                              |
| `PD01CR03` | 最近月度_最近一次还款日期       | 除C1        |                                              |
| `PD01CR04` | 最近月度_信息报告日期           | 除C1        |                                              |
| `PD01CS01` | 最近月度_剩余还款期数           | D1,R1,R2,R4 | 非分期账户返回空，R2账户指未出单大额分期期数 |
| `PD01CS02` | 最近月度_当期逾期期数           | D1,R1,R2,R4 |                                              |
...
-   *R2* 账户账户类型、额度
    ...
    -   多币卡在报告中体现为多个账户的，共享 *授信额度*、*已用额度*，但实际上大部分情况应合并账户
        -   依 `PD01AD04 == "CNY"` *币种* 剔除非本币账户
        -   依 `PD01AI04` *授信协议编号* 去重
        -   另外，也可以用 *授信协议* 中 `PD02AJ01` *授信额度* 计算额度
    -   信用卡额度说明
        -   白金卡
            -   额度一般在 10W 以上
            -   年费在 400 到 1000，免年费一般需要年消费达到 10W
        -   非白金卡
            -   额度一般在 5W 以下
            -   年费为 0 或满足一年 4-8 笔消费即可

-   *R2* 账户余额
    -   `PD01CJ01` *账户余额* 指账户下所有负债，包含已出单、未出单部分
    -   `PD01CJ02` *已用额度* 对应报送时 *已使用额度*，
    -   `PD01CJ03` *未出单* 指未在账单中体现部分，即报送时字段 *本期账单余额*
        -   大额专项分期一般不占用循环贷额度，仅需当期归还部分在 `PD01CJ04` *本月应还款* 中体现
    -   `PD01CJ04` *本月应还款* 指账户最低还款额
        -   对循环额度部分，最低还款额一般已用额度的
        -   但，大额专项分期当期应还是否占用 *已用额度* 取决于机构报送
...
-   *D1R41* 账户月负债：重点在规避还款方式、还款频率、期数等影响，`PD01CJ04` *当月应还* 不具备代表性
    ...
    -   否则在非按月还款时不代表月份数，需额外计算月份数
        ...
        -   剩余还款期数 `cb_max(mon_itv(PD01AR02, cb_max(PD01CR01, PD01CR04)), 1)`
            -   `PD01CR01` *月度月份* 可能为空
...
-   *R2* 账户月负债：重点在规避大额专项分期影响
    ...
    -   对普通贷记卡
        ...
        -   另，已用额度、本月应还波动较大，使用6月平均预估 `PD01CJ12` 更文稳健
...
| 最新表现   | 月度表现   | 其他段     | 含义             |
|------------|------------|------------|------------------|
| `PD01BD01` | `PD01CD01` |            | 账户状态         |
| `PD01BD03` | `PD01CD02` |            | 五级分类         |
| `PD01BD04` |            | `PD01ED01` | 还款状态         |
| `PD01BJ01` | `PD01CJ01` |            | 账户余额         |
| `PD01BJ02` | `PD01CJ04` |            | 实还金额         |
| `PD01BR01` |            | `PD01AR02` | 账户关闭日期     |
| `PD01BR02` | `PD01CR03` |            | 最近一次还款日期 |
| `PD01BR03` | `PD01CR04` | `PD01CR01` | 信息上报日期     |
