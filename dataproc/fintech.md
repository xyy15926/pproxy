...
| `PD01AS01` | 还款期数           | D1,R4      | 非分期还款账户返回空                              | 无空值，R1始终取 `--`，D1R4可取 `--` |
...
| `PD01CR01` | 最近月度_月份                   | 除C1        | `PC01CR02 == "--"` 时，此字段数据缺失                                         |
| `PD01CR02` | 最近月度_本月应还款日           | 除C1        | 可取 `--`                                                                     |
| `PD01CR03` | 最近月度_最近一次还款日期       | 除C1        |                                                                               |
| `PD01CR04` | 最近月度_信息报告日期           | 除C1        |                                                                               |
| `PD01CS01` | 最近月度_剩余还款期数           | D1,R1,R2,R4 | 非分期账户返回空，R2账户指未出单大额分期期数，可取 `--`                       |
...
-   *R2* 账户余额（不止本金）说明
    ...
    -   `PD01CJ03` *未出单* 指未在账单中体现部分，即报送时字段 *未出单大额专项分期余额*
        ...
        -   理论上应有 `PD01CJ01 >= PD01CJ02 + PD01CJ03`，但应未做强校验（数据核验中有少量账户不满足）
    -   `PD01CJ04` *本月应还款* 指账户最低还款额，对应报送时 *本期账单余额*
        -   包括
...
| `PD01BJ01` | 最新表现_账户余额         | all        | 非关闭、未激活账户                   |
| `PD01BJ02` | 最新表现_最近一次还款金额 | 除C1       | 持续更新、且有 **非月度报送** 账户   |
| `PD01BR01` | 最新表现_账户关闭日期     | all        | 关闭账户                             |
| `PD01BR02` | 最新表现_最近一次还款日期 | all        | 持续更新（除部分 *C*）、呆账账户非空 |
| `PD01BR03` | 最新表现_信息报告日期     | all        | **永远非空**，持续更新账户同月度段   |
...
-   最新表现不等于报送中 *非月度表现* 信息，*月度表现* 同样更新其取值
    ...
    -   五级分类：`PD01BD03`、`PD01CD02` 必同时空、非空
        -   但，非空时取值可能不一致
        -   取空情况
            -   关闭、转出、未激活、呆账账户
            -   *C* 类账户
    ...
    -   账户余额：摘要以 `PC01CJ01` 月度余额计算，仅 *R23* 账户以 `PD01BJ01` 最新余额补充计算
        -   `PD01BJ01` 最新余额在关闭、转出、未激活账户取空
        -   `PD01CJ01` 月度余额在关闭、转出、未激活、呆账、*C* 类账户取空
        -   `PD02AJ04` 为授信协议 *已用额度*
            -   对应 *D1R4* 账户，为 *借款金额* 汇总
            -   对应 *R413*、*R2* 账户，为 *余额*、*已使用额度* 汇总
...
-   `PCA:PD02` 授信协议信息：主体从数据提供机构获得的授信额度信息的载体
    ...
    -   可通过 `PD02AI03`、`PD01AI04` *授信协议标识* 与账户关联
        -   授信协议可对应多个信贷账户，即多个信贷账户共享授信额度
            -   *R4* 循环额分帐户还款账户
            -   多币种信用卡
        -   `PD01AD01` *账户类型*、`PD02AD02` *授信协议用途* 之间可任意对应，并无校验
...
-   账户额度来源于账户信息、授信协议
    ...
    -   `PD01AI04` *授信协议编号* 非空账户
        ...
        -   授信额度类型由 `PD02AD02` *授信协议用途* 确定
            -   取 `30` 时即对应 `PD01AJ03` *共享授信额度*
            -   取 `31` *独立授信额度* 应指该授信协议与机构内其他信用卡账户独立
                -   不代表该授信协议仅能对应单独账户
                -   数据核查表明存在对应多账户情况，但均为对应多币账户
    -   其他事项
        -   *授信额度* `PD02AJ01` 与 `PD01AJ01` 、`PD01AJ02`、`PD01AJ03` 无校验关系
        -   *授信限额* `PD02AJ03` 并不会在账户信息中任何位置体现，不对应 `PD01AJ03` *共享授信额度*
-   授信协议中 `PD02AJ04` *已用额度* 特殊，与账户额度不同
    -   对 *D1R4* 账户，`PD02AJ04` *已用额度* 指已借款金额，真正余额应取账户信息段中 `PD01CJ01`、`PD01BJ01`
    -   对 *R413* 账户，`PD02AJ04` *已用额度* 即指 `PD01CJ01`、`PD01BJ01` 
    -   对 *R2* 账户，`PD02AJ04` *已用额度* 指循环额度下已使用部分 `PD01CJ02`、`PD01BJ01`
...
-   账户摘要说明
    ...
    -   *R2* 账户
        -   `PC02FS02`：账户数量
        -   `PC02FJ01`：账户对应授信协议 `PD02AJ01` *授信额度*、账户 `PD01AJ03` *共享授信额度* 按序加总#TODO
            -   同上理论上，同一机构下多个无授信协议账户 `PD01AJ03` 同时取非 0，则上述计算逻辑会高估额度，但目前尚未发现
            -   但也可能低估额度，未计入无授信协议 *R2* 账户
        -   `PC02FJ04`：账户对应授信协议 `PD02AJ04` *已用额度*、 `PD01CJ02` *月度已用额度* 、`PD01BJ01` *最新余额* 按序加总
            -   此值仅包含已出单部分额度，则无授信协议账户应使用 `PD01CJ02` 计入
            -   且，*82-大额专项分期卡* 的已出单额度亦计入其中
            -   但，上述两种处理方式差别在于是否将 *82-大额专项分期卡* 呆账账户纳入，两种处理方式都有瑕疵
