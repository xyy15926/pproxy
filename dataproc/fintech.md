...
| `PD01AI01` | 账户编号           | all        | 征信系统分配，报告内唯一                          | **全空，已重新赋值**                            |
| `PD01AI02` | 业务管理机构代码   | all        | 机构在征信系统中唯一编码                          | **本机构账户置空，非本机构账户随机 2 位字母**   |
| `PD01AI03` | 账户标识           | 除C1       | 征信系统内全局唯一                                | **本机构账户有意义值，非本机构实际取 `******`** |
| `PD01AI04` | 授信协议编号       | 除C1       | 征信系统分配，报告内唯一                          | **大量空值**                                    |
...
-   各类型账户各字段取值说明（与报送规范、报文解读不一致处）
    ...
    -   `PD01AD05` *还款方式* 对 *D1* 账户存在大量空值，但对 *R41* 账户无空值
        -   *D1* 账户 `PD01AD05` 空值，**数据核验未找到规律**
            -   与 `PD01AR02` 账户 *到期日期* 无关
            -   与 `PD01BR02` 账户 *关闭日期* 无关
            -   与 `PD01BD01`、`PD01CD01` 账户状态无关
        -   但，同时类似的 `PD01AD06` *还款频率* 非空
    -   `PD01AD05` *贷款发放形式* 非空值仅取 `5-其他机构转入`
    -   `PD01AD07` *担保方式*
...
| `PC02DS02` | 逾期账户数                   |
| `PC02DS03` | 逾期 **月份数**              |
| `PC02DJ01` | 逾期单月最高逾期（透支）总额 |
| `PC02DS04` | 逾期最长逾期（透支）月数     |


-   逾期汇总信息段说明
    -   `PC02DS03` *逾期月份数* 是仅按月计数，多账户同月逾期仅增 1
...
-   报送内容、报送时点
    ...
    -   个人借贷交易信息
        -   借贷账户层：个人借贷账户信息
            ...
            -   账户关闭
            -   月结日：包含两个特殊时点
                -   “月结日首次上报存量账户” 时点：新机构（**新业务**）上报尚未结清账户
                -   “月结日账户关闭” 时点：月结日账户关闭
            -   收回逾期款项
                -   不包括月结日、账户关闭日
                -   需收回 **全部逾期款项**
    ...
    -   说明
        -   原则上上述事件 `T+1` 报送，但
            -   *R23* 账户激活前无需月结日按月报送
            -   已核销且无法按月生成数据账户可以仅在发生还款的月底报送

> - 数据采集规范中 “月结日首次上报存量账户” 仅针对 “新接入征信系统的数据提供机构”，事实上可适用于机构新业务先开展、后报送的情况
