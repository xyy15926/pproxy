...
-   数据量缺少
    -   解决方案
        -   *Data Augmentation* 数据增强：根据先验知识，在保留特点信息的前提下，对原始数据进行适当变换以达到扩充数据集的效果
...
-   *LDA* 线性判别分析：寻找投影方向，使得投影后样本尽可能按照原始类别分开，即寻找可以最大化类间距离、最小化类内距离的方向
    ...
    -   相较于 *PCA* 优点
        ...
        -   模型对噪声的稳健性更好
    -   缺点
        -   对数据分布有很强假设：各类服从正态分布、协方差相等，实际数据可能不满足
...
-   *Information Gain Ratio* 信息增益比：信息增益对原始信息熵的比值
