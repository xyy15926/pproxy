...
-   异步、同步模型的核心逻辑差异：非阻塞的提交任务、阻塞的执行任务至完成
    -   *Synchronous* 同步模型：**阻塞等待** 依赖任务执行完毕
        -   阻塞等待依赖任务会导致的 *CPU* 空转等待
            -   即使，内核主导、调度切换线程避免 *CPU* 空转
            -   但，用户任务自身不可感、逻辑上依然在阻塞等待
        -   故在同步模型中，按任务依赖关系拓扑排序执行可以缓解资源浪费
    -   *Asynchronous* 异步模型：提交任务，但不（即时）等待任务执行
        -   任务提交者 **监听** 被提交任务是否完成，同时继续后续指令执行
            -   借此，异步模型可规避同步模型阻塞等待的 *CPU* 空转
            -   而，任务自身主导的任务切换（协程切换）资源消耗相对更少
                -   即，异步模型与同步模型往往还有线程抢占式、协程协作式的控制权转移的差异
        -   实务中的监听的实现方式
            -   基于回调的异步模型：依赖任务完成后调用回调函数通知
            -   基于协程的异步模型：`await` 阻塞等待、转移控制权至运行时，由运行时统一监听、通知
    -   关于事件监听和超时
        -   事件监听在同步、异步模型均存在
            -   同步模型在内核主导任务切换时，同样需要通过信号确定是否任务是否就绪，本质上即事件监听
            -   或者说，内核线程切换是内核层面的、不受控异步
        -   但实务中，超时在同步模型无法对多任务有效控制
            -   内核线程调度由内核负责，难以为多个任务设置超时
            -   但，协程调度由运行时负责，为多个任务设置超时较方便
...
-   *Generator* 生成器：实现 `yield`、`resume` 机制，可 **主动暂停、满足条件后恢复执行**（并在其间传递数据）的计算单元
    -   生成器核心即：`yield` 让步主动暂停，被 `resume` 后在暂停位置恢复执行
    -   显然，生成器需要维护自身状态、存在多个执行入口点（以实现挂起后恢复执行）
        -   故，生成器逻辑上应为、可视为状态机
            -   状态：每个 `yield` 挂起、转移控制权处对应状态机各状态
            -   状态转移：状态机各状态维护协程内部状态、并转移自身状态
        -   运行时执行生成器，即匹配状态机状态、从对应状态开始执行
    -   生成器逻辑上不可独立（状态转移），其实现总是依赖于外部推动执行
        -   在 `yield` 点返回值、保存内部状态、挂起，控制权转移至外部
        -   外部 `resume` 触发生成器续作，生成器获取外部输入、恢复状态、继续执行

####    *Coroutine* 协程

-   *Coroutine* 协程：实现 `await`、`resume` 机制的生成器
    -   协程核心即：协程内 `await` 点、运行时 `resume` 点的协作式控制权相互转移
        -   `await <coroutine>` 机制（时点）：类似 `yield` 指示保存状态、转移控制权的挂起点
            -   `await` 不输出、接受值
            -   `await` 表示 **等待依赖的 `CoR` 完成**，并由运行时 `resume` 唤醒
            -   逻辑上 `await` 可视为：`loop` 循环内检查条件，不满足条件时 `yield NUL`、否则 `break`
        -   协程执行：在 `await` 点协程返回值、保存内部状态、挂起，在运行时中 **注册监听事件**
            -   监听事件即 `await` 对象（其他协程任务）
            -   事件回调即协程自身（后续执行）
        -   运行时执行：监听事件发生时，`resume` 恢复对应协程执行
            -   运行时核心即，**事件回调注册、事件监听、维护（协程）任务队列**
            -   监听事件发生时，对应协程任务将被加入就绪任务队列，而不是立刻执行
                -   事件发生时，运行时可能无控制权、其他协程在运行
                -   运行时获得控制权、执行时，将轮询就绪任务队列、`resume` 唤醒、恢复任务执行
    -   在生成器 `yield` 主动转移控制权基础上，协程引入运行时，将协程调度（恢复执行）交由运行时负责
        -   故，协程不适合直接由内核调度，总依赖于某个外部运行时监听事件、调度执行
        -   同样的，协程逻辑上不可独立（状态转移），其实现总是依赖于外部运行时
        -   事实上，“用户线程” 基本都按非抢占式的协程逻辑实现
            -   且，用户显然清楚何时需要暂停、等待依赖完成
            -   故，非抢占式的协程逻辑更合适 “用户线程” 模型
    -   相较于线程，协程调度完全由用户控制，可以视为、常实现为 *用户态轻量级线程*
        -   等待异步任务完成时自动挂起，避免空转浪费资源
...
-   协程与异步、事件监听
    -   协程模型主动挂起、调度恢复的非抢占式特性，简化了异步编程模型的实现
        -   异步的核心即，非阻塞的提交任务、不等待任务执行
            -   `await` 依赖的异步任务完成后被调度、自动恢复后续执行，无需设置回调函数
            -   使得需要使用 *异步+回调* 的代码可以使用看似同步方式写出
            -   而，事实上协程运行时核心即异步编程模型的 *事件监听、回调*
        -   另外，`await` 机制使得异步模型可以从上至下的执行任务，无需编排任务执行顺序
            -   `await` 点协程挂起，自上而下、由根到叶的 `await` *拉动* 依赖任务执行
            -   否则，根据任务依赖关系，按拓扑顺序、自下而上、由叶到根的执行任务
    -   协程框架（运行时）实现 *I/O*、定时任务监听，**实现单线程（单个 *CPU*）的 “并行能力”**
        -   即，协程模型总需要有类似 `submit` 非阻塞提交任务、`gather` 阻塞获取多个任务结果的能力
            -   否则，协程模型与顺序的函数调用无差异
            -   或者说，协程模型 **通过引入运行时获得 “并行能力” 的生成器**
        -   而，单线程并行能力只能来源于的非 *CPU* 计算的任务处理
            -   故，最底层协程必然为计时、*I/O* 等无 *CPU* 的后台任务
            -   即，运行时最底层事件监听总为计时、*I/O*（网络、文件）等系统调用
            -   异步协程框架的实现总与 *I/O* 任务、定时任务相关，提供与系统调用相关相关的 *I/O*、计时协程任务
...
-   *Ordering* 有序性：程序（指令）严格按代码先后顺序执行
    -   *Instruction Reorder* 指令重排序：指令执行顺序重排，在保证程序最后执行结果不变情况下提升效率
        ...
        -   但，某线程中指令重排序可能带来其他线程 **依赖逻辑错误、执行结果异常**
            ...
            -   指令重排序仅能考虑、保证单线程场景的执行结果不变
        -   典型即，变量 `var` 被用作线程间同步的信号
            -   逻辑上，要求线程 `1` 内条件动作执行完成后 `var` 置位
            -   但，线程 `1` 中可能因指令重排序导致 `var` 置位后，条件动作才执行
            -   则，依赖、检查 `var` 置位情况，而开始执行的线程将被错误启动
    -   （需）通过 *锁内原子变量* 的内存顺序约束、`volatile` 机制下，避免指令重排序影响多线程执行结果
...
-   内存顺序：原子操作相关的内存访问（指令执行）顺序
    -   `Relaxed` 无顺序约束：指令执行顺序可被任意优化
        -   适合任意原子操作
    -   `Release(+ Store)` 约束：配合的 `Store` 操作前的指令不可重排序至当前 `Store` 操作后
    ...
    -   `Acquire(+ Load)` 约束：配合的 `Load` 操作后指令不可重排序至当前 `Load` 操作前
        -   即，**确保原子变量 `Load` 载入前，后续逻辑不可执行**
            -   逻辑同 `Release(+ Store)`
        -   特别的，`Load` 载入线程（需）知悉所有相关线程 `Store` 存储（同一）原子变量
        -   `Acquire` 约束只应用于可执行 `Load` 操作的指令
...
-   关于 `Release`、`Acquire` 内存顺序约束使用使用的说明
    -   仅在线程之间存在 **条件（线程）、目标（线程）依赖时成对使用** `Release(+ Store)`、`Acquire(+ Load)`（否则 `Relaxed` 即可）
        -   即，在 **前序指令是操作条件后跟** `Store + Release` 约束
        -   并配合，在 **在目标动作前加** `Load + Acquire` 约束
            -   以确保原子变量的 **值变化** 可作为信号跨线程同步状态
        -   即，`Release + Acquire` 对 **值变化影响执行分支** 应被使用，考虑引用计数
            -   增加不影响关联值是否应被丢弃，可 `Relaxed`
            -   但，减少时需判断是否减少至 0、丢弃，则减少后 `Release + Store`、丢弃前 `Acquire + Load`
    -   特别的，对同时包含 `Load`、修改、`Store` （按顺序依次）语义的指令
        -   指令自身因包含修改语义，本身即可为 `Store` 的前序条件
...
impl<T> Drop for Arc<T> {
    fn drop(&mut self) {
        ...
        // 可替换为 `_ = inner.rc.load(Ordering::Acquire);`
...
> - `Arc` Cloning：<https://doc.rust-lang.net.cn/nomicon/arc-mutex/arc-clone.html>
> - `Arc` Dropping：<https://doc.rust-lang.net.cn/nomicon/arc-mutex/arc-drop.html>
