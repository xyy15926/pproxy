...
-   *Coroutine* 协程同时包含状态维护逻辑、调度实现概念
    -   逻辑上：协程与子程序对应，差别在于是否维护自身状态
        -   子程序：无状态保存，同样重入得到同样结果
        -   协程：有保存状态，重入会改变协程状态，结果可能不同
    -   实现上：进程、线程、协程可以看作是控制权（资源、调度）逐渐从系统转移到自身
        -   进程：独立任务、独立资源
        -   线程：（内核或运行时）抢占式调度、进程内线程共享资源
            -   线程作为任务执行的实体，可以认为是子程序的具体执行
            -   即将，线程、子程序是相对应的执行实体、逻辑实体
        -   协程：协作式调度（主动让出 *CPU* 执行、资源）
    -   其他说明
        -   进程移植性差在于，基于进程分支多进程和 *Win* 平台模型有很大冲突，难以使用

####    *Subroutine* 子程序

-   *Subroutine*、*Procedure*、*Function*、*Routine*、*Method* 子程序：打包为整体、用于执行特定任务的指令集序列
    -   子程序是依赖可重入能力的弱化版本
        -   一旦唤醒，于起始点开始执行
        -   一旦退出，子程序结束
...
-   *Coroutine* 协程：实现 `yield <VAL>` 机制，可暂停、恢复、并在其间传递数据的计算单元
    -   协程可以通过 *`yield` 让步* 调用其他协程暂时退出，之后可在退出位置恢复执行
        -   即，暗含协程可包含多个入口点
        -   允许在不同入口点暂停、开始执行程序
    -   局部状态维护：协程实例将维护自身局部状态，保持上次退出时状态
        -   则，协程被唤醒时状态可能不同
        -   可能同时有多个给定协程实例
        -   即，协程将原在子程序外、输入状态管理工作交由自身逻辑维护
    -   广义上，现代编程环境中的类对象（或具体的 “状态机”）即可视为协程
    -   狭义上，协程指为 **非抢占式** 多任务产生子程序的程序组件，允许执行过程中挂起、恢复
        -   即，具体实现上对协程的定义更强调 **暂停、恢复带来的非抢占式调度** 特性
        -   即，强调协程对异步代码逻辑上的简化
...
-   子程序、协程的关系
    -   子程序可以看作时协程的特例：无状态保存，同样重入得到同样结果
        -   只有一个状态，每次进入时局部状态重置
        -   唯一入口点
    -   协程可视为子程序的组成部分：有保存状态，重入会改变协程状态，结果可能不同
        -   协程维护自身状态，所以 **逻辑上不独立运行**，应该是作为被调用对象
        -   对于每次返回部分结果值的协程（也称生成器迭代器），可以直接视为类似链表之类的数据结构
            -   在某些语言中可以所有数据都是类，从这个角度这也都是统一的
        -   现代指令集通常提供对调用栈的指令支持，便于实现可递归调用的子程序
            -   在提供续体的语言环境（如 *Scheme*），恰好可用此抽象状态表示实现协程
...
-   协程逻辑上不可独立（状态转移），其实现总是依赖于外部运行时
    -   外部运行时负责维护协程任务队列
        ...
        -   运行时选择队列中下个协程开始执行
    -   协程可以简化异步代码的实现
        -   协程使得需要使用 *异步+回调* 的代码可以使用看似同步方式写出
            -   协程本身只涉及 **状态保存、过程重入**，和并发、异步无关系
            -   但，协程本身蕴含的状态保存使得状态切换、任务切换（控制权转移）几乎无成本，适合高并发任务
        -   即，引入 `await <CoR>` 机制（时点）
            -   `await` 类似 `yield` 指示保存状态、转移控制权的挂起点
            -   但，`await` 不输出、接受值，而蕴含 **等待依赖的异步的任务 `CoR` 完成** 的语义（异步运行时负责唤醒）
        -   协程在线程中调度完全由用户控制，可以视为 *用户态轻量级线程*
            -   避免陷入 **无效内核级别上下文切换** 造成的性能损失
            -   较线程在 *IO* 密集任务上性能上更好
    -   *Stackless Coroutine* 无栈协程：无独立调用栈、依赖运行时调用栈的协程实现
        -   无栈协程一般在 `yield`、`await` 处将控制权转移给运行时
            -   在 `yield <VAL>` 处保存内部状态、返回值、挂起
            -   在 `await <CoR>` 处保存内部状态、等待其他协程返回值、挂起
                -   从协程角度看，这是调用其他协程而不是退出
                -   但实际上，**各协程之间是对称、平等的，而非子程序调用中 *主调-被调* 关系**
        -   无栈协程可视为是状态机
    ...
    -   *Stackful Coroutine* 有栈协程：拥有独立调用栈的协程实现
        -   有栈协程在切换时需要保存、恢复整个调用栈，开销较大
        -   有栈协程中协程切换类似子程序调用，各协程之间不对称、协程

> - 有栈协程和无栈协程：<https://zhuanlan.zhihu.com/p/347445164>
...
-   并发编程中为保证数据（共享变量）一致性，要满足 3 条件、或解决 3 个问题
    ...
    -   可见性：多个线程访问同一变量时，若某线程修改变量值，其他线程可以立刻看到修改后值
